<template>
  <div id="AlbumDetail">
      ...
      <template #default>
        ...
        <div class="albumBtnList">
          <div @click="toFollow" class="btnItem">
            <img style="width: 17px" v-if="otherInfo.isSub" src="~images/common/follow.png" alt="">
            <img style="width: 12px; opacity: 0.5; margin-right: 4px" v-else src="~images/common/unfollow.png" alt="">
        ...
        <div class="singerList">
          ...
          <span @click="router.push('/SingerDetail?id='+item.id)" v-for="item in dataInfo.artists" :key="item.id">
            {{item.name}}&nbsp;&nbsp;
          </span>
        </div>
        <div class="albumTime">
    ...
    <detailNav :watch="true" style="margin-bottom: 20px" :routerKey="'albumIndex'" :list="navList"></detailNav>
...
import { getAlbumDetail, getAlbumInfo, followAlbum } from "@/network/AlbumDetail/albumDetail";
      ...
      if (useLogin()) {
        followAlbum({
          ...
          if (res.data.code == 200) {
            ElNotification({
              message: data.otherInfo?.isSub ? '已取消收藏专辑' : '收藏专辑成功',
              type: 'success',
