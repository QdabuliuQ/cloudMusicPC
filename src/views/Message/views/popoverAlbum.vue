  ...
  <div class="popoverAlbum">
    <div v-if="uid != userInfo.userId" class="unselfContainer">
      ...
      >
        ...
        <div
          @click="router.push('/AlbumDetail?id=' + msg.album.id)"
          class="albumInfo"
        >
          <el-avatar
            shape="square"
            :size="40"
            :fit="'cover'"
            :src="msg.album.blurPicUrl"
          />
          <div class="otherInfo">
            <div class="title">
              <span class="target">专辑</span>{{ msg.album.name }}
            </div>
            <div class="artists">
              <span
                @click.stop="router.push('/SingerDetail?id=' + item.id)"
                v-for="item in msg.album.artists"
                :key="item.id"
              >
                {{ item.name }}&nbsp;&nbsp;
              </span>
